 ...
 * @brief Core data structures and domain matching logic for the Conference
 * Assignment Tool.
 */
 ...
 */
struct Parameters {
    int minReviewsPerSubmission = -1; ///< Minimum number of reviewers required
				      ///< per submission (-1 if not set)
    int maxReviewsPerReviewer = -1;   ///< Maximum number of submissions a
				      ///< reviewer can handle (-1 if not set)
    int primaryReviewerExpertise =
	1; ///< Weight of primary reviewer expertise (default: 1)
    int secondaryReviewerExpertise =
	0; ///< Weight of secondary reviewer expertise (default: 0)
    int primarySubmissionDomain =
	1; ///< Weight of primary submission domain (default: 1)
    int secondarySubmissionDomain =
	0; ///< Weight of secondary submission domain (default: 0)
};
...
struct Control {
     ...
     * only.
     * - 3: All primary and secondary domains for both submissions and
     * reviewers.
     */
     ...
     * - 0: No risk analysis.
     * - 1: Checks if assignment is still possible if any single reviewer is
     * removed.
     * - K (K>1): Checks if assignment is still possible if any K reviewers are
     * removed.
     */
    int riskAnalysis = 0;

    std::string outputFileName =
	"output.csv"; ///< Name of the output file (default: output.csv)
};
...
/**
 * @brief Determines if a reviewer can review a submission based on domain
 * matching.
 *
 ...
 * - Mode 2: Primary and secondary submission domains vs primary reviewer
 * expertise.
 ...
 *
 * @complexity Time: O(1) — constant number of comparisons regardless of input
 * size.
...
inline int domainsMatch(const Submission &sub, const Reviewer &rev, int mode) {
	...
	if (primaryMatch)
	    return sub.primary;
	if (subHasSecondary && rev.primary == sub.secondary)
	    return sub.secondary;
    } else if (mode == 3) {
	if (primaryMatch)
	    return sub.primary;
	if (subHasSecondary && rev.primary == sub.secondary)
	    return sub.secondary;
	if (revHasSecondary && rev.secondary == sub.primary)
	    return sub.primary;
	if (subHasSecondary && revHasSecondary &&
	    rev.secondary == sub.secondary)
	    return sub.secondary;
    }
