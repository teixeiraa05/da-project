...
void Menu::handleLoadFile() {
	...
	std::cout << "Warning: No data loaded. Check if the file exists and "
		     "has the correct format (.csv)."
		  << std::endl;
    } else {
	...
	std::cout << "Data loaded successfully. "
		  << this->data.submissions.size() << " submissions and "
		  << this->data.reviewers.size() << " reviewers found."
		  << std::endl;
    }
}
...
void Menu::handleShowSubmissions() {
	...
	if (assignmentGenerated && solver) {
	    ...
	    std::cout << " [" << (int)flowCap.first << "/"
		      << (int)flowCap.second << "]";
	}
	std::cout << ", Primary Domain: " << sub.primary
		  << ", Secondary Domain: " << sub.secondary << std::endl;
    }
}
...
void Menu::handleShowReviewers() {
	...
	std::cout << ", Primary Domain: " << rev.primary
		  << ", Secondary Domain: " << rev.secondary << std::endl;
    }
}
...
void Menu::handleShowParameters() {
    ...
    std::cout << "\nParameters:\n";
    std::cout << "Minimum reviews per submission: "
	      << data.params.minReviewsPerSubmission << std::endl;
    std::cout << "Maximum reviews per reviewer: "
	      << data.params.maxReviewsPerReviewer << std::endl;
}
...
void Menu::handleShowControl() {
    ...
    std::cout << "\nControl Settings:\n";
    std::cout << "Generate Assignments: " << data.control.generateAssignments
	      << std::endl;
    std::cout << "Risk Analysis: " << data.control.riskAnalysis << std::endl;
    std::cout << "Output File Name: " << data.control.outputFileName
	      << std::endl;
}
...
void Menu::handleShowLastAssignments() {
    ...
    if (!assignmentGenerated || !solver) {
	std::cout << "No assignments generated yet. Run an algorithm first."
		  << std::endl;
	return;
    }
...
void Menu::handleRunTests() {
	...
	std::cout
	    << "Error: Input and output directory paths cannot be empty.\n";
	return;
    }
	...
	int ret = std::system(
	    "g++ -std=c++17 -Wall -I Code/include Code/src/*.cpp -o run_tests");
	if (ret != 0) {
	...
	std::string expectedPath =
	    (fs::path(outputDir) / ("output_" + inputName)).string();

	if (!fs::exists(generatedPath)) {
	    std::cout << "  FAILED  " << inputName
		      << " (no output file: " << outputFileName << ")\n";
	    failed++;
	    continue;
	}
	if (!fs::exists(expectedPath)) {
	    std::cout << "  SKIPPED " << inputName
		      << " (no expected file: output_" << inputName << ")\n";
	    skipped++;
	    continue;
	}
	...
	while (true) {
	    ...
	    if (hasGen != hasExp) {
		if (firstDiff.empty())
		    firstDiff = "  Line count differs (generated: " +
				std::to_string(genLineNum) +
	    ...
	    if (genLine != expLine) {
		if (firstDiff.empty())
		    firstDiff = "  Line " + std::to_string(genLineNum) +
				" differs:\n"
				"    got:      \"" +
				genLine +
				"\"\n"
...
void Menu::handleRiskAnalysis() {
	...
	std::cout << "Found " << riskyReviewers.size()
		  << " risky reviewer(s): ";
	for (size_t i = 0; i < riskyReviewers.size(); i++) {
	    std::cout << riskyReviewers[i]
