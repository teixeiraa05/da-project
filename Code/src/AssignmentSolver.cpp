...
void AssignmentSolver::exportAssignments(const std::string &filename) const {
	...
	for (Edge<int> *edge : subVertex->getAdj()) {
		...
		if (revIndex < 0 ||
		    revIndex >= static_cast<int>(data.reviewers.size()))
		    continue;
		const Reviewer &actualRev = data.reviewers[revIndex];

		int matchDomainId = domainsMatch(
		    actualSub, actualRev, data.control.generateAssignments);
		matches.push_back({actualSub.submissionId, actualRev.reviewerId,
				   matchDomainId});
	    }
	}
	...
	if (missing > 0) {
	    missingReviews.push_back(
		{actualSub.submissionId, actualSub.primary, missing});
	}
    }

    // --- BLOCK 1: Submission Order ---
    // matches is already naturally sorted by submission index from the loop
    // above
	...
	for (const auto &record : missingReviews) {
	    outFile << record.subId << ", " << record.domain << ", "
		    << record.missingCount << "\n";
	}
    }
...
std::vector<int>
AssignmentSolver::riskAnalysis(const std::string &filename) const {
    ...
    int sourceId = 0;
    int sinkId = getSinkId();
    int totalRequired = static_cast<int>(data.submissions.size() *
    ...
    if (s_orig) {
	for (auto e : s_orig->getAdj())
	    initialFlow += static_cast<int>(e->getFlow());
    }
	...
	for (size_t i = 0; i < data.reviewers.size(); i++) {
	    ...
	    tempGraph.setEdgeWeight(reviewerNodeId, sinkId,
				    data.params.maxReviewsPerReviewer);
	}
    }
...
int AssignmentSolver::getSinkId() const {
    return static_cast<int>(data.submissions.size() + data.reviewers.size() +
			    1);
}
...
void AssignmentSolver::generateGraphviz(const std::string &filename) const {
	...
	if (id == sourceId) {
	    ...
	    if (subIdx >= 0 &&
		subIdx < static_cast<int>(data.submissions.size())) {
		label = "Sub " +
			std::to_string(data.submissions[subIdx].submissionId);
	    } else {
		label = "Sub ?";
	    }
	    ...
	    if (revIdx >= 0 &&
		revIdx < static_cast<int>(data.reviewers.size())) {
		label =
		    "Rev " + std::to_string(data.reviewers[revIdx].reviewerId);
	    } else {
		label = "Rev ?";
	    }
	    fillcolor = "lightblue";
	...
	for (auto edge : v->getAdj()) {
	    // Assuming your capacity is stored in getWeight() as per your
	    // previous snippet
	    if (edge->getWeight() > 0) {
		out << "    \"" << v->getInfo() << "\" -> \""
...
void AssignmentSolver::printAssignments() const {
    ...
    // Use a vector for O(1) access and fewer allocations - Senior Mentor
    // optimization
	...
	for (Edge<int> *edge : subVertex->getAdj()) {
		...
		std::cout << "source -> [" << actualSub.submissionId << " : "
			  << (int)nodeAgg[subNodeId].first << "/"
			  << (int)nodeAgg[subNodeId].second << "] -> ["
			  << actualRev.reviewerId << " : "
			  << (int)nodeAgg[revNodeId].first << "/"
			  << (int)nodeAgg[revNodeId].second << "] -> sink"
			  << std::endl;
		found = true;
	    }
	}
...
std::pair<double, double> AssignmentSolver::getReviewerFlow(int revId) const {
    ...
    if (revVertex) {
	for (auto e : revVertex->getAdj()) {
	    if (e->getDest()->getInfo() ==
		static_cast<int>(data.reviewers.size() +
