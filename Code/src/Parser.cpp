...
ConferenceData Parser::parseFile(const std::string &filename) {
    ...
    */
	...
	std::cerr << "Error: input file must have a .csv extension."
		  << std::endl;
	return data;
    }
	...
	if (line[0] == '#') {
		...
		currentSection = "CTRL";
	    // If it's a comment line that doesn't match any section header
	    // (like #Id, Title, ...), we just ignore it
	    continue;
	}
	...
	// Dispatch to the appropriate parsing function based on the current
	// section

	if (currentSection == "SUB")
	    parseSubmission(tokens, data);
	else if (currentSection == "REV" && tokens.size() >= 4)
	    parseReviewer(tokens, data);
	else if (currentSection == "PARAM" && tokens.size() >= 2)
	    parseParameter(tokens, data);
	else if (currentSection == "CTRL" && tokens.size() >= 2)
	    parseControl(tokens, data);
	else {
	    std::cerr << "Warning: Unrecognized line in input file: " << line
		      << std::endl;
	}
    }
	...
	std::cerr << "Error: Validation failed for the parsed data. Please "
		     "check the input file for issues."
		  << std::endl;
...
void Parser::parseSubmission(const std::vector<std::string> &tokens,
			     ConferenceData &data) {
    Submission sub;
    if (tokens.size() < 5) {
	std::cerr
	    << "Error: Invalid submission entry, expected at least 5 fields."
	    << std::endl;
	return;
    }
    ...
    if (tokens.size() > 5)
	sub.secondary = toInt(tokens[5]);

    data.submissions.push_back(sub);
}

void Parser::parseReviewer(const std::vector<std::string> &tokens,
			   ConferenceData &data) {
    if (tokens.size() < 4) {
	std::cerr
	    << "Error: Invalid reviewer entry, expected at least 4 fields."
	    << std::endl;
	return;
    }
    ...
    if (tokens.size() > 4)
	rev.secondary = toInt(tokens[4]);

    data.reviewers.push_back(rev);
}

void Parser::parseParameter(const std::vector<std::string> &tokens,
			    ConferenceData &data) {
    if (tokens.size() < 2) {
	std::cerr
	    << "Error: Invalid parameter entry, expected at least 2 fields."
	    << std::endl;
	return;
    }
	...
	data.params.minReviewsPerSubmission = val;
    else if (key == "MaxReviewsPerReviewer")
	data.params.maxReviewsPerReviewer = val;
    else if (key == "PrimaryReviewerExpertise")
	data.params.primaryReviewerExpertise = val;
    else if (key == "SecondaryReviewerExpertise")
	data.params.secondaryReviewerExpertise = val;
    else if (key == "PrimarySubmissionDomain")
	data.params.primarySubmissionDomain = val;
    else if (key == "SecondarySubmissionDomain")
	data.params.secondarySubmissionDomain = val;
}

void Parser::parseControl(const std::vector<std::string> &tokens,
			  ConferenceData &data) {
    if (tokens.size() < 2) {
	std::cerr << "Error: Invalid control entry, expected at least 2 fields."
		  << std::endl;
	return;
    }
	...
	data.control.generateAssignments = toInt(val);
    else if (key == "RiskAnalysis")
	data.control.riskAnalysis = toInt(val);
    else if (key == "OutputFileName")
	data.control.outputFileName = val;
}
...
std::vector<std::string> Parser::splitLine(const std::string &line) {
    ...
    std::string cur;
    bool inQuote =
	false; // there can be commas inside quotes, we should ignore them
...
bool Parser::validate(ConferenceData &data) {
	...
	if (submissionIds.count(sub.submissionId)) {
	    std::cerr << "Validation Error: Duplicate submission ID "
		      << sub.submissionId << std::endl;
	    return false;
	}
	...
	if (reviewerIds.count(rev.reviewerId) > 0) {
	    std::cerr << "Validation Error: Duplicate reviewer ID "
		      << rev.reviewerId << std::endl;
	    return false;
	}
